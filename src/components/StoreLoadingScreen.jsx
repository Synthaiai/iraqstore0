...
export default function StoreLoadingScreen() {
  ...
  return <div className="store-loading" role="status" aria-live="polite"><div className="store-loading__card"><div className="store-loading__logo-wrap"><span className="store-loading__orbit" /><img className="store-loading__mark" src="/logo.jpg" alt="IRAQI STORE" width="72" height="72" /></div><strong>جاري تجهيز المتجر</strong><span>نحمّل المنتجات والصور حتى تظهر الصفحة كاملة</span><div className="store-loading__progress" aria-label={`اكتمل التحميل ${progress}%`}><i style={{ width: `${progress}%` }} /></div><small>{progress}%</small></div></div>;
}
